...
def sendemail(Order, csv_url):
    ...
    for orderOne in Order['order_details']:
        ...
        header += '<th width="25%">COMPANY CODE</th>'
        header += '<th width="25%">SALSE ORDER DATE</th>'
        header += '</tr>'
        header += '</thead>'
        header += '<tbody>'
        header += '<tr>'
        ...
        for order in orderOne["orders"]:
            ...
            header +=                     '<td width="10%">'+str(format(float(order["product"]["mrp1"]),".2f"))+'</td>'
            header +=                     '<td width="10%">'+str(order["quantity"])+'</td>'
            header +=                     '<td width="10%">'+str(format(float(order["product"]["mrp1"])*float(order["quantity"]),".2f"))+'</td>'
            header +=                 '</tr>'
            header +=             '</tbody>'
        header +=           ' </table>'
    ...
    header  +=          '<th width="20%" ></th>'
    header  +=          '<th width="30%" ></th>'
    header  +=                 '</tr>'
    header  +=             '</thead>'
    header  +=             '<tbody>'
    header  +=                 '<tr>'
    header  +=                     '<td width="50%">'
    header  +=                         '<strong>Total Quantity</strong>'
    header  +=                     '</td>'
    header  +=                     '<td width="50%">'
    header  +=                        '<strong>'+str(Order["total_quantity"])+'</strong>'
    header  +=                     '</td>'
    header  +=                 '</tr>'
    header  +=                 '<tr>'
    header  +=                     '<td width="50%">'
    ...
    header  +=                     '</td>'
    header  +=                     '<td width="50%">'
    header  +=                        '<strong>'+str(format(Order["total"],".2f"))+'</strong>'
    header  +=                     '</td>'
    header  +=                 '</tr>'
    header  +=             ' </tbody>'
    header  +=       '</table>'
    ...
    header  +=   '<p style="color:red;text-align: left;text-transform: none;">Note:- Discount applicable as per company Policy, Taxes as applicable</p>'
    header  += '</div>'
    header  += '</body>'
    header  += '</html>'
    html_content = header
