...
const startLoading = (state, action) => R.merge(state, { loading: true, cancelToken: action.cancelToken });
const stopLoading = state => R.merge(state, { loading: false, cancelToken: null });
...
export const loadStarted = (tag, cancelToken) => ({ type: `LOAD_${tag}_STARTED`, cancelToken });
export const loadEnded = tag => ({ type: `LOAD_${tag}_ENDED` });
export const loadSuccess = (tag, params) => ({ type: `LOAD_${tag}_SUCCESS`, ...params });
export const loadFailure = (tag, error) => ({ type: `LOAD_${tag}_FAILURE`, error });
